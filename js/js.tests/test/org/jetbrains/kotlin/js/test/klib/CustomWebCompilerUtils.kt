...
private fun createCustomWebCompilerSettings(
    ...
    override val customCompiler: CustomWebCompiler by lazy {
        CustomWebCompiler(
            listOf(
                artifacts.resolve("kotlin-compiler-embeddable", extension = "jar"),
                artifacts.resolve("kotlin-stdlib", extension = "jar")
            )
        )
    }
}
...
private sealed interface CustomWebCompilerArtifacts {
    ...
    fun resolve(baseName: String, extension: String): File
    ...
    private class Resolvable(private val artifactsDir: File, private val version: String) : CustomWebCompilerArtifacts {
        ...
        override fun resolve(baseName: String, extension: String) =
            artifactsDir.resolve("$baseName-$version.$extension")
                .also { assertTrue(it.exists(), "Artifact $baseName is not found: $it") }
    }

    private class Unresolvable(val reason: String) : CustomWebCompilerArtifacts {
        override val areArtifactsAvailable = false
        override fun resolve(baseName: String, extension: String) = fail(reason)
    }
